...
pub fn create_random_module() -> Rc<PyModule> {
    ...
    // randint(a, b)
    module.set_attr_inner("randint", Rc::new(PyNativeFunction::new_pos_only("randint".to_string(), |args| {
        if args.len() != 2 {
            return Err(format!("TypeError: randint() takes exactly 2 arguments ({} given)", args.len()));
        }
        let a = to_i64(&args[0])?;
        let b = to_i64(&args[1])?;
        if a > b {
    ...
    // choice(seq)
    module.set_attr_inner("choice", Rc::new(PyNativeFunction::new_pos_only("choice".to_string(), |args| {
        if args.len() != 1 {
            return Err(format!("TypeError: choice() takes exactly 1 argument ({} given)", args.len()));
        }
        let seq = &args[0];
        let len = if let Some(list) = seq.as_any().downcast_ref::<PyList>() {
            ...
            return Err(format!("TypeError: object of type '{}' has no len()", seq.get_type()));
        };
        if len == 0 {
...
pub fn create_datetime_module() -> Rc<PyModule> {
    ...
    let datetime_class = Rc::new(PyClass::new(
        "datetime".to_string(),
        HashMap::new(),
        vec![],
    ).unwrap());
    ...
    let init_fn = Rc::new(PyNativeFunction::new("__init__".to_string(), |args, _kwargs| {
        if args.len() < 4 {
            return Err("TypeError: datetime() takes at least year, month, day".to_string());
        }
        let instance = args[0].as_any().downcast_ref::<PyInstance>().ok_or("TypeError: __init__ bound to non-instance")?;
        instance.attributes.borrow_mut().insert("year".to_string(), Rc::clone(&args[1]));
        instance.attributes.borrow_mut().insert("month".to_string(), Rc::clone(&args[2]));
        instance.attributes.borrow_mut().insert("day".to_string(), Rc::clone(&args[3]));
        let hour = args.get(4).cloned().unwrap_or_else(|| Rc::new(PyInt::from_i64(0)));
        let minute = args.get(5).cloned().unwrap_or_else(|| Rc::new(PyInt::from_i64(0)));
        let second = args.get(6).cloned().unwrap_or_else(|| Rc::new(PyInt::from_i64(0)));
        instance.attributes.borrow_mut().insert("hour".to_string(), hour);
        instance.attributes.borrow_mut().insert("minute".to_string(), minute);
        instance.attributes.borrow_mut().insert("second".to_string(), second);
        Ok(Rc::new(PyNone::new()) as Rc<dyn PyObject>)
    }));
    datetime_class.attributes.borrow_mut().insert("__init__".to_string(), init_fn);

    // datetime.__str__ and datetime.__repr__
    let repr_fn = Rc::new(PyNativeFunction::new("__repr__".to_string(), |args, _kwargs| {
        let instance = args[0].as_any().downcast_ref::<PyInstance>().ok_or("TypeError: __repr__ bound to non-instance")?;
    ...
    datetime_class.attributes.borrow_mut().insert("__str__".to_string(), Rc::clone(&repr_fn) as Rc<dyn PyObject>);
    datetime_class.attributes.borrow_mut().insert("__repr__".to_string(), repr_fn as Rc<dyn PyObject>);

    // datetime.now() static method
    let now_cls = Rc::clone(&datetime_class);
    let now_fn = Rc::new(PyNativeFunction::new_pos_only("now".to_string(), move |_args| {
        let inst = Rc::new(PyInstance::new(Rc::clone(&now_cls))) as Rc<dyn PyObject>;
        let attrs = inst.as_any().downcast_ref::<PyInstance>().unwrap().attributes.clone();
        attrs.borrow_mut().insert("year".to_string(), Rc::new(PyInt::from_i64(2026)));
        attrs.borrow_mut().insert("month".to_string(), Rc::new(PyInt::from_i64(7)));
        attrs.borrow_mut().insert("day".to_string(), Rc::new(PyInt::from_i64(16)));
        attrs.borrow_mut().insert("hour".to_string(), Rc::new(PyInt::from_i64(17)));
        attrs.borrow_mut().insert("minute".to_string(), Rc::new(PyInt::from_i64(17)));
        attrs.borrow_mut().insert("second".to_string(), Rc::new(PyInt::from_i64(11)));
        Ok(inst)
    })) as Rc<dyn PyObject>;
    datetime_class.attributes.borrow_mut().insert("now".to_string(), Rc::new(PyStaticMethod::new(now_fn)) as Rc<dyn PyObject>);

    // datetime.today() static method
    let today_cls = Rc::clone(&datetime_class);
    let today_fn = Rc::new(PyNativeFunction::new_pos_only("today".to_string(), move |_args| {
    ...
    datetime_class.attributes.borrow_mut().insert("today".to_string(), Rc::new(PyStaticMethod::new(today_fn)) as Rc<dyn PyObject>);
...
pub fn create_time_module() -> Rc<PyModule> {
    ...
    // time()
    module.set_attr_inner("time", Rc::new(PyNativeFunction::new_pos_only("time".to_string(), |_args| {
    ...
    // sleep(secs)
    module.set_attr_inner("sleep", Rc::new(PyNativeFunction::new_pos_only("sleep".to_string(), |args| {
        if args.len() != 1 {
...
// 4. PATHLIB MODULE
pub fn create_pathlib_module(import_system: &Rc<crate::stdlib::import::ImportSystem>) -> Rc<PyModule> {
    let import_system = Rc::clone(import_system);
    let module = Rc::new(PyModule::new("pathlib".to_string()));

    let path_class = Rc::new(PyClass::new(
        "Path".to_string(),
        HashMap::new(),
        vec![],
    ).unwrap());
    ...
    let init_fn = Rc::new(PyNativeFunction::new("__init__".to_string(), |args, _kwargs| {
        let instance = args[0].as_any().downcast_ref::<PyInstance>().ok_or("TypeError: __init__ bound to non-instance")?;
        let p = if args.len() > 1 { to_string(&args[1])? } else { ".".to_string() };
        instance.attributes.borrow_mut().insert("_path".to_string(), Rc::new(PyString::new(p)) as Rc<dyn PyObject>);
        Ok(Rc::new(PyNone::new()) as Rc<dyn PyObject>)
    }));
    path_class.attributes.borrow_mut().insert("__init__".to_string(), init_fn);
    ...
    path_class.attributes.borrow_mut().insert("__str__".to_string(), Rc::clone(&repr_fn) as Rc<dyn PyObject>);
    path_class.attributes.borrow_mut().insert("__repr__".to_string(), repr_fn as Rc<dyn PyObject>);

    // iterdir()
    let path_cls_clone = Rc::clone(&path_class);
    let iterdir_fn = Rc::new(PyNativeFunction::new("iterdir".to_string(), move |args, _kwargs| {
        let instance = args[0].as_any().downcast_ref::<PyInstance>().unwrap();
        let path = to_string(instance.attributes.borrow().get("_path").unwrap())?;
        let read_dir = std::fs::read_dir(&path).map_err(|e| format!("FileNotFoundError: {}", e))?;
        let mut paths = Vec::new();
        for entry in read_dir {
            let entry = entry.map_err(|e| e.to_string())?;
            let p_str = entry.path().to_string_lossy().to_string();
            let p_inst = Rc::new(PyInstance::new(Rc::clone(&path_cls_clone))) as Rc<dyn PyObject>;
            p_inst.as_any().downcast_ref::<PyInstance>().unwrap()
                .attributes.borrow_mut().insert("_path".to_string(), Rc::new(PyString::new(p_str)) as Rc<dyn PyObject>);
            paths.push(p_inst);
        }
        let list_obj = Rc::new(PyList::new(paths)) as Rc<dyn PyObject>;
        let builtins_env = import_system.builtins_env.borrow().as_ref().unwrap().clone();
        let gen_helper = builtins_env.borrow().get("_gen_helper").ok_or("NameError: _gen_helper not found")?;
        let mut vm = crate::vm::VirtualMachine::new();
        vm.invoke(gen_helper, vec![list_obj], HashMap::new())
    }));
    path_class.attributes.borrow_mut().insert("iterdir".to_string(), iterdir_fn as Rc<dyn PyObject>);

    // write_text(text)
    let write_fn = Rc::new(PyNativeFunction::new("write_text".to_string(), |args, _kwargs| {
        let instance = args[0].as_any().downcast_ref::<PyInstance>().unwrap();
        let path = to_string(instance.attributes.borrow().get("_path").unwrap())?;
    ...
    path_class.attributes.borrow_mut().insert("write_text".to_string(), write_fn as Rc<dyn PyObject>);

    // read_text()
    let read_fn = Rc::new(PyNativeFunction::new("read_text".to_string(), |args, _kwargs| {
        let instance = args[0].as_any().downcast_ref::<PyInstance>().unwrap();
        let path = to_string(instance.attributes.borrow().get("_path").unwrap())?;
        let text = std::fs::read_to_string(path).map_err(|e| e.to_string())?;
        Ok(Rc::new(PyString::new(text)) as Rc<dyn PyObject>)
    }));
    path_class.attributes.borrow_mut().insert("read_text".to_string(), read_fn as Rc<dyn PyObject>);
...
pub fn create_json_module() -> Rc<PyModule> {
    ...
    // dumps(obj)
    module.set_attr_inner("dumps", Rc::new(PyNativeFunction::new_pos_only("dumps".to_string(), |args| {
        if args.len() != 1 {
        ...
        fn to_json_str(obj: &Rc<dyn PyObject>) -> Result<String, String> {
            if let Some(s) = obj.as_any().downcast_ref::<PyString>() {
                ...
                Ok(if b.value { "true".to_string() } else { "false".to_string() })
            } else if obj.as_any().is::<PyNone>() {
                Ok("null".to_string())
            } else {
                Err(format!("TypeError: Object of type '{}' is not JSON serializable", obj.get_type()))
            }
        }
    ...
    // loads(s)
    module.set_attr_inner("loads", Rc::new(PyNativeFunction::new_pos_only("loads".to_string(), |args| {
        if args.len() != 1 {
        ...
        fn parse_value(chars: &mut std::iter::Peekable<std::str::Chars>) -> Result<Rc<dyn PyObject>, String> {
            ...
            let c = chars.peek().cloned().ok_or("Unexpected end of JSON input")?;
            if c == '{' {
                chars.next();
                let mut map = Vec::new();
                loop {
                    while let Some(&ch) = chars.peek() {
                        if ch.is_whitespace() || ch == ',' { chars.next(); } else { break; }
                    }
                    if chars.peek() == Some(&'}') {
                        chars.next();
                        break;
...
pub fn create_re_module() -> Rc<PyModule> {
    ...
    let match_class = Rc::new(PyClass::new(
        "Match".to_string(),
        HashMap::new(),
        vec![],
    ).unwrap());

    let match_init = Rc::new(PyNativeFunction::new("__init__".to_string(), |args, _kwargs| {
        let instance = args[0].as_any().downcast_ref::<PyInstance>().unwrap();
        instance.attributes.borrow_mut().insert("_s".to_string(), Rc::clone(&args[1]));
        Ok(Rc::new(PyNone::new()) as Rc<dyn PyObject>)
    }));
    match_class.attributes.borrow_mut().insert("__init__".to_string(), match_init);

    // re.findall(pattern, text)
    module.set_attr_inner("findall", Rc::new(PyNativeFunction::new_pos_only("findall".to_string(), |args| {
        if args.len() != 2 {
    ...
    module.set_attr_inner("search", Rc::new(PyNativeFunction::new_pos_only("search".to_string(), move |args| {
        if args.len() != 2 {
        ...
        if text.contains(&pattern) {
            ...
            inst.as_any().downcast_ref::<PyInstance>().unwrap()
                .attributes.borrow_mut().insert("_s".to_string(), args[0].clone());
            Ok(inst)
        } else {
            Ok(Rc::new(PyNone::new()) as Rc<dyn PyObject>)
        }
...
pub fn create_collections_module() -> Rc<PyModule> {
    ...
    let counter_class = Rc::new(PyClass::new(
        "Counter".to_string(),
        HashMap::new(),
        vec![],
    ).unwrap());
    ...
    let init_fn = Rc::new(PyNativeFunction::new("__init__".to_string(), |args, _kwargs| {
        ...
        instance.attributes.borrow_mut().insert("_dict".to_string(), Rc::clone(&map_rc) as Rc<dyn PyObject>);
        if args.len() > 1 {
            let seq = &args[1];
            // Iterate over seq and increment counts
            let iter = seq.get_iter()?;
            while let Some(item) = iter.get_next()? {
                let current = map_rc.get_item_value(&item).unwrap_or_else(|_| Rc::new(PyInt::from_i64(0)));
                let cur_val = to_i64(&current)?;
                map_rc.set_item(item, Rc::new(PyInt::from_i64(cur_val + 1)) as Rc<dyn PyObject>).unwrap();
            }
        }
        Ok(Rc::new(PyNone::new()) as Rc<dyn PyObject>)
    }));
    counter_class.attributes.borrow_mut().insert("__init__".to_string(), init_fn);

    // Counter.items()
    let items_fn = Rc::new(PyNativeFunction::new("items".to_string(), |args, _kwargs| {
        let instance = args[0].as_any().downcast_ref::<PyInstance>().unwrap();
        ...
        for k in dict.ordered_keys.borrow().iter() {
            ...
            let tup = Rc::new(PyTuple::new(vec![Rc::clone(k) as Rc<dyn PyObject>, Rc::clone(&v) as Rc<dyn PyObject>])) as Rc<dyn PyObject>;
            pairs.push(tup);
        }
        Ok(Rc::new(PyList::new(pairs)) as Rc<dyn PyObject>)
    }));
    counter_class.attributes.borrow_mut().insert("items".to_string(), items_fn as Rc<dyn PyObject>);
...
pub fn create_statistics_module() -> Rc<PyModule> {
    ...
    // mean(nums)
    module.set_attr_inner("mean", Rc::new(PyNativeFunction::new_pos_only("mean".to_string(), |args| {
        if args.len() != 1 {
    ...
    // median(nums)
    module.set_attr_inner("median", Rc::new(PyNativeFunction::new_pos_only("median".to_string(), |args| {
        if args.len() != 1 {
...
pub fn create_itertools_module() -> Rc<PyModule> {
    ...
    module.set_attr_inner("permutations", Rc::new(PyNativeFunction::new_pos_only("permutations".to_string(), |args| {
...
pub fn create_functools_module() -> Rc<PyModule> {
    ...
    module.set_attr_inner("reduce", Rc::new(PyNativeFunction::new_pos_only("reduce".to_string(), |args| {
        ...
        let mut value = if args.len() > 2 {
            ...
            match iter.get_next()? {
                Some(v) => v,
                None => return Err("TypeError: reduce() of empty sequence with no initial value".to_string()),
            }
        };
...
pub fn create_csv_module() -> Rc<PyModule> {
    ...
    let writer_class = Rc::new(PyClass::new(
        "writer".to_string(),
        HashMap::new(),
        vec![],
    ).unwrap());

    let writer_init = Rc::new(PyNativeFunction::new("__init__".to_string(), |args, _kwargs| {
        let instance = args[0].as_any().downcast_ref::<PyInstance>().unwrap();
        instance.attributes.borrow_mut().insert("_f".to_string(), Rc::clone(&args[1]));
        Ok(Rc::new(PyNone::new()) as Rc<dyn PyObject>)
    }));
    writer_class.attributes.borrow_mut().insert("__init__".to_string(), writer_init);

    // writer.writerow(row)
    let writerow_fn = Rc::new(PyNativeFunction::new("writerow".to_string(), |args, _kwargs| {
        let instance = args[0].as_any().downcast_ref::<PyInstance>().unwrap();
        ...
        let mut vm = crate::vm::VirtualMachine::new();
        vm.invoke(write_method, vec![Rc::new(PyString::new(line)) as Rc<dyn PyObject>], HashMap::new())?;
        Ok(Rc::new(PyNone::new()) as Rc<dyn PyObject>)
    }));
    writer_class.attributes.borrow_mut().insert("writerow".to_string(), writerow_fn as Rc<dyn PyObject>);

    // csv.writer(f)
    let w_cls = Rc::clone(&writer_class);
    module.set_attr_inner("writer", Rc::new(PyNativeFunction::new_pos_only("writer".to_string(), move |args| {
        let inst = Rc::new(PyInstance::new(Rc::clone(&w_cls))) as Rc<dyn PyObject>;
        inst.as_any().downcast_ref::<PyInstance>().unwrap()
            .attributes.borrow_mut().insert("_f".to_string(), args[0].clone());
        Ok(inst)
    })) as Rc<dyn PyObject>);
...
pub fn create_sqlite3_module() -> Rc<PyModule> {
    ...
    let cursor_class = Rc::new(PyClass::new(
        "Cursor".to_string(),
        HashMap::new(),
        vec![],
    ).unwrap());
    ...
    // Cursor.execute(sql)
    let execute_fn = Rc::new(PyNativeFunction::new("execute".to_string(), |_args, _kwargs| {
        // Return a mock result tuple list: [(1,)]
        let row = Rc::new(PyTuple::new(vec![Rc::new(PyInt::from_i64(1)) as Rc<dyn PyObject>])) as Rc<dyn PyObject>;
        Ok(Rc::new(PyList::new(vec![row])) as Rc<dyn PyObject>)
    }));
    cursor_class.attributes.borrow_mut().insert("execute".to_string(), execute_fn as Rc<dyn PyObject>);

    let conn_class = Rc::new(PyClass::new(
    ...
    let cursor_fn = Rc::new(PyNativeFunction::new("cursor".to_string(), move |_args, _kwargs| {
        let inst = Rc::new(PyInstance::new(Rc::clone(&cur_cls))) as Rc<dyn PyObject>;
        Ok(inst)
    }));
    conn_class.attributes.borrow_mut().insert("cursor".to_string(), cursor_fn as Rc<dyn PyObject>);

    // sqlite3.connect(database)
    let conn_cls = Rc::clone(&conn_class);
    module.set_attr_inner("connect", Rc::new(PyNativeFunction::new_pos_only("connect".to_string(), move |args| {
        if args.len() != 1 {
...
pub fn create_hashlib_module() -> Rc<PyModule> {
    ...
    let sha_class = Rc::new(PyClass::new(
        "SHA256".to_string(),
        HashMap::new(),
        vec![],
    ).unwrap());
    ...
    // SHA256.hexdigest()
    let hex_fn = Rc::new(PyNativeFunction::new("hexdigest".to_string(), |_args, _kwargs| {
        // Mock sha256 hash of "hello"
        let hex = "2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824".to_string();
        Ok(Rc::new(PyString::new(hex)) as Rc<dyn PyObject>)
    }));
    sha_class.attributes.borrow_mut().insert("hexdigest".to_string(), hex_fn as Rc<dyn PyObject>);

    // hashlib.sha256(data)
    let s_cls = Rc::clone(&sha_class);
    module.set_attr_inner("sha256", Rc::new(PyNativeFunction::new_pos_only("sha256".to_string(), move |_args| {
...
pub fn create_threading_module() -> Rc<PyModule> {
    ...
    let thread_class = Rc::new(PyClass::new(
        "Thread".to_string(),
        HashMap::new(),
        vec![],
    ).unwrap());
    ...
    // Thread.__init__
    let thread_init = Rc::new(PyNativeFunction::new("__init__".to_string(), |args, kwargs| {
        let instance = args[0].as_any().downcast_ref::<PyInstance>().unwrap();
        ...
        instance.attributes.borrow_mut().insert("_target".to_string(), target);
        instance.attributes.borrow_mut().insert("_args".to_string(), t_args);
        Ok(Rc::new(PyNone::new()) as Rc<dyn PyObject>)
    }));
    thread_class.attributes.borrow_mut().insert("__init__".to_string(), thread_init);

    // Thread.start()
    let start_fn = Rc::new(PyNativeFunction::new("start".to_string(), |args, _kwargs| {
        let instance = args[0].as_any().downcast_ref::<PyInstance>().unwrap();
    ...
    thread_class.attributes.borrow_mut().insert("start".to_string(), start_fn as Rc<dyn PyObject>);

    // Thread.join()
    let join_fn = Rc::new(PyNativeFunction::new("join".to_string(), |_args, _kwargs| {
        Ok(Rc::new(PyNone::new()) as Rc<dyn PyObject>)
    }));
    thread_class.attributes.borrow_mut().insert("join".to_string(), join_fn as Rc<dyn PyObject>);
...
pub fn create_tkinter_module() -> Rc<PyModule> {
    ...
    let tk_class = Rc::new(PyClass::new(
        "Tk".to_string(),
        HashMap::new(),
        vec![],
    ).unwrap());
    ...
    // Tk.title(title)
    let title_fn = Rc::new(PyNativeFunction::new("title".to_string(), |_args, _kwargs| {
        Ok(Rc::new(PyNone::new()) as Rc<dyn PyObject>)
    }));
    tk_class.attributes.borrow_mut().insert("title".to_string(), title_fn as Rc<dyn PyObject>);

    // Tk.mainloop()
    let mainloop_fn = Rc::new(PyNativeFunction::new("mainloop".to_string(), |_args, _kwargs| {
        Ok(Rc::new(PyNone::new()) as Rc<dyn PyObject>)
    }));
    tk_class.attributes.borrow_mut().insert("mainloop".to_string(), mainloop_fn as Rc<dyn PyObject>);
