...
pub fn create_math_module() -> Rc<PyModule> {
    ...
    module.set_attr_inner(
        "pi",
        Rc::new(PyFloat::new(std::f64::consts::PI)) as Rc<dyn PyObject>,
    );
    module.set_attr_inner(
        "e",
        Rc::new(PyFloat::new(std::f64::consts::E)) as Rc<dyn PyObject>,
    );
    module.set_attr_inner(
        "tau",
        Rc::new(PyFloat::new(std::f64::consts::TAU)) as Rc<dyn PyObject>,
    );
    module.set_attr_inner(
        "inf",
        Rc::new(PyFloat::new(f64::INFINITY)) as Rc<dyn PyObject>,
    );
    ...
    module.set_attr_inner(
        ...
        Rc::new(PyNativeFunction::new_pos_only(
            ...
            |args| {
                if args.len() != 2 {
                    return Err(format!(
                        "TypeError: remainder() takes exactly 2 arguments ({} given)",
                        args.len()
                    ));
                }
                let x = to_f64(&args[0])?;
    ...
    module.set_attr_inner(
        ...
        Rc::new(PyNativeFunction::new_pos_only(
            ...
            |args| {
                if args.len() != 1 {
                    return Err(format!(
                        "TypeError: trunc() takes exactly one argument ({} given)",
                        args.len()
                    ));
                }
                let x = to_f64(&args[0])?;
    ...
    module.set_attr_inner(
        "fabs",
        Rc::new(PyNativeFunction::new_pos_only("fabs".to_string(), |args| {
            if args.len() != 1 {
                return Err(format!(
                    "TypeError: fabs() takes exactly one argument ({} given)",
                    args.len()
                ));
            }
            let x = to_f64(&args[0])?;
    ...
    module.set_attr_inner(
        ...
        Rc::new(PyNativeFunction::new_pos_only(
            ...
            |args| {
                if args.len() != 1 {
                    return Err(format!(
                        "TypeError: degrees() takes exactly one argument ({} given)",
                        args.len()
                    ));
                }
                let x = to_f64(&args[0])?;
    ...
    module.set_attr_inner(
        ...
        Rc::new(PyNativeFunction::new_pos_only(
            ...
            |args| {
                if args.len() != 1 {
                    return Err(format!(
                        "TypeError: radians() takes exactly one argument ({} given)",
                        args.len()
                    ));
                }
                let x = to_f64(&args[0])?;
        ...
        Rc::new(PyNativeFunction::new(
            ...
            |args, kwargs| {
                if args.len() < 2 || args.len() > 4 {
                    return Err(format!(
                        "TypeError: isclose() takes 2-4 arguments ({} given)",
                        args.len()
                    ));
                }
                let a = to_f64(&args[0])?;
                ...
                let rel_tol = if args.len() >= 3 {
                    ...
                    kwargs
                        .get("rel_tol")
                        .map_or(1e-9, |v| to_f64(v).unwrap_or(1e-9))
                };
                let abs_tol = if args.len() >= 4 {
                    ...
                    kwargs
                        .get("abs_tol")
                        .map_or(0.0, |v| to_f64(v).unwrap_or(0.0))
                };
                let diff = (a - b).abs();
    ...
    module.set_attr_inner(
        ...
        Rc::new(PyNativeFunction::new_pos_only(
            ...
            |args| {
                if args.len() != 1 {
                    return Err(format!(
                        "TypeError: isfinite() takes exactly one argument ({} given)",
                        args.len()
                    ));
                }
                let x = to_f64(&args[0])?;
    ...
    module.set_attr_inner(
        ...
        Rc::new(PyNativeFunction::new_pos_only(
            ...
            |args| {
                if args.len() != 1 {
                    return Err(format!(
                        "TypeError: isinf() takes exactly one argument ({} given)",
                        args.len()
                    ));
                }
                let x = to_f64(&args[0])?;
    ...
    module.set_attr_inner(
        ...
        Rc::new(PyNativeFunction::new_pos_only(
            ...
            |args| {
                if args.len() != 1 {
                    return Err(format!(
                        "TypeError: isnan() takes exactly one argument ({} given)",
                        args.len()
                    ));
                }
                let x = to_f64(&args[0])?;
    ...
    module.set_attr_inner(
        "fmod",
        Rc::new(PyNativeFunction::new_pos_only("fmod".to_string(), |args| {
            if args.len() != 2 {
                return Err(format!(
                    "TypeError: fmod() takes exactly 2 arguments ({} given)",
                    args.len()
                ));
            }
            let x = to_f64(&args[0])?;
    ...
    module.set_attr_inner(
        "gcd",
        Rc::new(PyNativeFunction::new_pos_only("gcd".to_string(), |args| {
            if args.len() != 2 {
                return Err(format!(
                    "TypeError: gcd() takes exactly 2 arguments ({} given)",
                    args.len()
                ));
            }
            let a = to_i64(&args[0])?;
    ...
    module.set_attr_inner(
        "lcm",
        Rc::new(PyNativeFunction::new_pos_only("lcm".to_string(), |args| {
            if args.len() != 2 {
                return Err(format!(
                    "TypeError: lcm() takes exactly 2 arguments ({} given)",
                    args.len()
                ));
            }
            let a = to_i64(&args[0])?;
        ...
        Rc::new(PyNativeFunction::new_pos_only(
            ...
            |args| {
                ...
                for i in 2..=n {
                    result = result
                        .checked_mul(i)
                        .ok_or_else(|| "OverflowError: factorial too large".to_string())?;
                }
    ...
    module.set_attr_inner(
        "comb",
        Rc::new(PyNativeFunction::new_pos_only("comb".to_string(), |args| {
            if args.len() != 2 {
                return Err(format!(
                    "TypeError: comb() takes exactly 2 arguments ({} given)",
                    args.len()
                ));
            }
            let n = to_i64(&args[0])?;
            ...
            for i in 1..=k {
                result = result
                    .checked_mul(n - k + i)
                    .ok_or_else(|| "OverflowError: comb too large".to_string())?;
                result /= i;
            }
            Ok(Rc::new(PyInt::from_i64(result)))
        })) as Rc<dyn PyObject>,
    ...
    module.set_attr_inner(
        "perm",
        Rc::new(PyNativeFunction::new_pos_only("perm".to_string(), |args| {
            if args.len() != 2 {
                return Err(format!(
                    "TypeError: perm() takes exactly 2 arguments ({} given)",
                    args.len()
                ));
            }
            let n = to_i64(&args[0])?;
            ...
            for i in (n - k + 1)..=n {
                result = result
                    .checked_mul(i)
                    .ok_or_else(|| "OverflowError: perm too large".to_string())?;
            }
            Ok(Rc::new(PyInt::from_i64(result)))
        })) as Rc<dyn PyObject>,
    );
    ...
    module.set_attr_inner(
        "dist",
        Rc::new(PyNativeFunction::new_pos_only("dist".to_string(), |args| {
            if args.len() != 2 {
                return Err(format!(
                    "TypeError: dist() takes exactly 2 arguments ({} given)",
                    args.len()
                ));
            }
            ...
            if p.len() != q.len() {
                return Err(
                    "ValueError: both points must have the same number of dimensions".to_string(),
                );
            }
    ...
    module.set_attr_inner(
        ...
        Rc::new(PyNativeFunction::new(
            ...
            move |args, kwargs| {
                ...
                let result = if base == math_e { x.ln() } else { x.log(base) };
    ...
    module.set_attr_inner(
        ...
        Rc::new(PyNativeFunction::new_pos_only(
            ...
            |args| {
                if args.len() != 1 {
                    return Err(format!(
                        "TypeError: frexp() takes exactly one argument ({} given)",
                        args.len()
                    ));
                }
                let x = to_f64(&args[0])?;
    ...
    module.set_attr_inner(
        "fsum",
        Rc::new(PyNativeFunction::new_pos_only("fsum".to_string(), |args| {
            if args.len() != 1 {
                return Err(format!(
                    "TypeError: fsum() takes exactly one argument ({} given)",
                    args.len()
                ));
            }
    ...
    module.set_attr_inner(
        "prod",
        Rc::new(PyNativeFunction::new("prod".to_string(), |args, kwargs| {
            if args.is_empty() || args.len() > 2 {
                return Err(format!(
                    "TypeError: prod() takes 1-2 arguments ({} given)",
                    args.len()
                ));
            }
            let iter = args[0].get_iter()?;
            let start = if args.len() >= 2 {
                ...
                kwargs
                    .get("start")
                    .map_or(1.0, |v| to_f64(v).unwrap_or(1.0))
            };
    ...
    module.set_attr_inner(
        "ulp",
        Rc::new(PyNativeFunction::new_pos_only("ulp".to_string(), |args| {
            if args.len() != 1 {
                return Err(format!(
                    "TypeError: ulp() takes exactly one argument ({} given)",
                    args.len()
                ));
            }
            let x = to_f64(&args[0])?;
