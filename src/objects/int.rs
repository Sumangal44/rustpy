...
impl PyObject for PyInt {
    ...
    fn add(&self, other: Rc<dyn PyObject>) -> Option<Rc<dyn PyObject>> {
        ...
        } else if let Some(c) = other
            .as_any()
            .downcast_ref::<crate::objects::complex::PyComplex>()
        {
            let val = self.value.to_f64().unwrap_or(0.0);
            Some(Rc::new(crate::objects::complex::PyComplex::new(
                val + c.real,
                c.imag,
            )))
        } else {
    ...
    fn eq(&self, other: Rc<dyn PyObject>) -> Option<Rc<dyn PyObject>> {
        ...
        } else if let Some(b) = other
            .as_any()
            .downcast_ref::<crate::objects::bool::PyBool>()
        {
            let b_val = if b.value {
                BigInt::one()
            } else {
                BigInt::zero()
            };
    ...
    fn get_attr(&self, attr: &str) -> Result<Rc<dyn PyObject>, String> {
        ...
        match attr {
            ...
            "to_bytes" => Ok(Rc::new(PyNativeFunction::new(
                "to_bytes".to_string(),
                move |args, _kwargs| {
                    let length = if let Some(l) =
                        args.get(0).and_then(|a| a.as_any().downcast_ref::<PyInt>())
                    {
                    ...
                    let _signed = args
                        ...
                        .and_then(|a| {
                            if let Some(b) =
                                a.as_any().downcast_ref::<crate::objects::bool::PyBool>()
                            {
                                Some(b.value)
                            } else {
                                None
            ...
            "as_integer_ratio" => Ok(Rc::new(PyNativeFunction::new_pos_only(
                ...
                move |_args| {
                    ...
                    Ok(Rc::new(crate::objects::tuple::PyTuple::new(vec![num, den]))
                        as Rc<dyn PyObject>)
                },
            ))),
            "conjugate" => Ok(Rc::new(PyNativeFunction::new_pos_only(
                "conjugate".to_string(),
                move |_args| Ok(Rc::new(PyInt::new(val.clone())) as Rc<dyn PyObject>),
            ))),
